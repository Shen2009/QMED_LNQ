  ...
  return (
    <SafeAreaView style={[s.root, {backgroundColor: C.bg}]}>
      <View style={[s.header, {borderBottomColor: C.border}]}>
        ...
        <Text style={[s.title, {color: C.text}]}>{strings.personalInfoTitle}</Text>
        <View style={{width: 38}} />
      </View>
      <View style={s.center}>
        <MaterialIcons name="person-outline" size={64} color={C.teal} />
        <Text style={[s.comingSoon, {color: C.text}]}>{strings.comingSoon}</Text>
        <Text style={[s.sub, {color: C.textSub}]}>{strings.personalInfoSub}</Text>
      </View>
    </SafeAreaView>
  );
}
